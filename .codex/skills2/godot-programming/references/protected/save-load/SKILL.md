...
Keep Save and Settings as separate contracts even when they share small file helpers:

- a Save service owns versioned player-progress snapshots, slot publication, migration, and recovery;
- a Settings service owns validated application preferences, defaults, update methods, and immutable
  snapshots for consumers;
- `ConfigFile` remains an internal persistence detail, not a public arbitrary `String` key/value API;
- commands such as `save_slot()`, `load_slot()`, and `update_audio_mix()` are typed methods; signals
  such as `save_completed` or `settings_changed` report completed facts;
- neither service retains live scene Nodes or mutates a scene while parsing.

Promote either service to an Autoload only when it must survive scene replacement or serve
independently owned scenes. Let the application composition root initialize Settings before applying
its snapshot to Audio and other consumers. Do not make Save and Settings call sibling Autoloads from
`_ready()`, and do not merge them with routing, audio, or game state into one manager.
