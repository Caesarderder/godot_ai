...
- A second-pass Save, Settings, Audio, or SceneRouter Autoload has current evidence for application
  lifetime, a narrow typed API, explicit failure semantics, and an acyclic initialization dependency.
- Cross-service reactions are wired by the application composition root; sibling Autoload `_ready()`
  methods do not hide ordering dependencies or form a service locator.
- SceneRouter accepts allowlisted route IDs rather than arbitrary resource paths and rejects stale or
  overlapping transition generations.
