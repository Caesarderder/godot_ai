...
3. Prefer a feature-first hybrid layout: co-locate each feature's scene, script, and feature-owned
   resources while keeping truly shared code, UI, levels, and governed assets in explicit roots.
4. Add only settings and autoloads required by the current playable feature.
5. Validate with the repository's Godot binary and existing tests.
