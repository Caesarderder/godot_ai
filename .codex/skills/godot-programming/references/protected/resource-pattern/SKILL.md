...
`ResourceSaver.save(resource, path)` writes trusted authored or application-generated definitions to
a `.tres` (text) or `.res` (binary) file. Do not use Resource loading as the default user save-game or
import format: persisted player data needs the version, bounds, migration, recovery, and untrusted
input validation owned by `save-load`.
...
## Architecture hardening boundary

During a second architecture pass, use Resources for typed authored definitions such as settings
defaults, route catalogs, audio catalogs, and gameplay tuning. Keep loaded definitions immutable by
default. User settings and save slots belong under `user://` through validated persistence formats;
authoritative run/session state belongs to an owned runtime model. Do not turn one `GameConfig`
Resource into a service locator or mutable application database.
...
- [ ] `.tres` / `.res` files never loaded from untrusted external or user-controlled sources
- [ ] User settings and save slots use the validated persistence formats owned by `save-load`
