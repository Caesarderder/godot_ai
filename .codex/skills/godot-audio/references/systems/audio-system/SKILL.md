...
During a second architecture pass, promote an Audio service to an Autoload only when music, bus mix,
or browser-unlock state must survive route changes. Keep positional audio and feature SFX with their
scene owner. Apply one validated Settings snapshot through a typed method; do not let Audio discover
or query a Settings Autoload in `_ready()`. The application composition root owns that wiring.

The global Audio service must not retain scene Nodes, own gameplay timing, or become a generic sound
event bus. Commands such as `play_music()` and `apply_mix()` are methods. Signals are reserved for
facts such as unlock or playback-state changes and are never emitted every frame.
