...
5. Before writing production gameplay code, use `$godot-architecture` to record the minimum target
   file tree, scene/owner boundaries, typed signal contracts, and Resource versus runtime-state plan.
   Each planned `.tscn`, `.gd`, and `.tres` must have a current milestone responsibility; avoid
   speculative systems and empty scaffolding.
...
Implement the accepted `$godot-architecture` blueprint rather than discovering every boundary inside
one root script. Choose a scene root that composes the run controller, authoritative runtime state,
player, world/challenge, HUD, and feedback owners. Route semantic input intent to the controller;
publish factual typed state changes to read-only HUD/FX consumers. Use direct typed calls for owned
commands and local signals for occurrences; do not introduce a broad event bus.
...
- the implemented `.tscn`, `.gd`, signal, and Resource boundaries match the recorded blueprint or
  document an evidence-driven revision;
