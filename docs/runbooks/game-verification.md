...
援军 smoke 必须使用独立新 Chrome profile，从标题页经基地真实触控进入 1-1，再通过每次真实
结算主 CTA 连续推进 1-2、1-3 与 1-4；战斗窗口持续练习英雄卡技能输入，不允许改存档、加速
引擎或调用测试后门。通过条件是 IndexedDB 中真实 `save_v1.json` 的 schema/content 合同正确，
`cleared_stages` 精确包含前三关、1-4 尝试一次但未通关、onboarding 已推进到研究突破，并且
无运行时异常、非预期 console error 或网络失败。随后必须在 844×390 真实网格完成研究所选址、
确认信号招募页的免费基础图纸十连 claim-once 且不直接新增角色，再在 5 秒研究所施工到点后
无需切页即可验收，逐张研发冲锋/装甲永久角色，并执行装甲前排/冲锋次位的两次真实编队命令。
脚本保存首战、高墙、信号、施工、两次研发和编队转折；重复技能卡
触控只证明真实输入路径被执行，主动技能是否被玩家理解仍由真人协议交叉确认。编队完成后必须
点击已渲染的反攻 CTA，轮询三张真实英雄卡，并以 `stage_1_4` 尝试次数为 2 且进入
`cleared_stages` 作为破坎成功条件；胜利页需保留援军贡献与双线成长出口。反攻后先进入
冲锋/装甲二星二选一，两个不可逆按钮必须同时位于 844×390 首屏；随后进入工厂。新档工业库存
已经被研究所耗尽时，工厂必须先让玩家亲手领取已解锁的“新游补给礼包”，并在 IndexedDB 中
精确得到 30 工业材料，再展示三种可建资源设施。工业段继续完成真实 3D 网格选址、5 秒施工、
原页验收和首批产出收取；通过条件包含礼包 claim-once、设施等级与位置、订单清空、资源正增量
和具体角色 `star == 2`。零库存设施不得继续提供可点击的收取行动。

同一新档随后必须通过 1-5 Boss、只产生一次 1-5 尝试并解锁但不自动挑战 2-1；首章结算的
“领取阵营起手十连”必须直接落到可领取状态，不得再受隐藏指挥官等级门槛阻断。脚本需真实领取
免费十连、持久化独立 claim key、让玩家从两名同评级候选中明确选择阵营核心，并打开所选核心的
科技图纸；截至此处 2-1 尝试仍须为 0。研发、入队、三场阵营实战证明和专属碎片升星属于后续
30–60 分钟连续 Web 门禁，不能用 headless 领域旅程冒充浏览器证据。
...
1. Gman 单人完成 1-1 至 1-3，并在真实战斗中至少主动释放一次技能；
2. 1-4 首战失败后，不经口头提示找到研究所恢复路径；
3. 在信号招募接收免费基础图纸十连，主动建成研究所并逐张研发装甲与冲锋，再编入反攻队；
4. 反攻 1-4，并能用本局反馈解释两名援军如何改变结果；
5. 任选资源设施投产、收取首批后勤，在冲锋/装甲二星中做一次自主选择；
6. 以所选路线处理 1-5 巨炮，失败时能找到非付费恢复办法；
7. 通关后能说明“攻城暴露需求—工厂供养永久军团—军团继续攻城”的双向循环。

自动 Web 连续旅程还必须在同一隔离新档中验证：1-5 只增加一次尝试且确实通关、结算页报告
角色贡献与压炮次数、`highest_unlocked_stage` 推进到 2-1、点击章节解锁 CTA 后仍不增加 2-1
尝试次数。第二章必须先显示推荐战力、能力比、风险级别与培养/试探选择；导航到侦察页不等于
玩家授权开战。自动重复技能输入只证明输入链和结算可达，不能记作“理解炮击倒计时”。

#### 五人盲测协议

每名参与者使用新存档、目标横屏手机或等效 844×390 触控环境，预计 20–30 分钟。开始前只说：
“请像刚下载的新游戏一样游玩；想到什么可以说出来。我不会教你怎么过关。”观察者不得指出按钮、
解释数值或提醒下一步；只有设备故障、页面无法响应或参与者明确结束时才介入。

开始前让参与者在设置中自愿开启本地试玩报告；不同意时照常观察，不把开启报告作为参与条件。
结束后由参与者主动下载 JSON，并确认文件由其交给研究者；游戏不会上传。观察记录至少包含：

- 首 30 秒认为当前目标是什么；
- 每次停顿超过 15 秒时所看区域与尝试动作；
- 1-4 首败后认为失败原因是什么；
- 十连后是否能独立完成两人编队；
- 选择冲锋/装甲路线时比较了哪些结果；
- 1-5 首次炮击后认为发生了什么；
- 是否愿意继续、最想继续做什么、最无聊或困惑的一刻。

结束访谈按固定顺序使用中立问题，不提示正确答案：

1. “你刚才主要在做什么？”
2. “第一次打不过 1-4 时，你觉得为什么？”
3. “冲锋和装甲的成长选项有什么区别？你为什么选这个？”
4. “1-5 的巨炮怎样处理？你从哪里知道的？”
5. “如果再玩十分钟，你最想做什么？为什么？”

将报告的 `next_missing_milestone` 与录像/观察交叉定位；`milestone_intervals_seconds` 只表示行为间隔，
不能单独解释原因。验收沿用项目合同：4/5 无口头提示完成闭环，4/5 能解释 1-4 与成长选择，
3/5 主动表达继续游玩意愿；任一主要段落出现 90 秒无新判断、有效操作、清晰反馈或期待兑现，
必须建立具体摩擦项并复测，不能用平均完成时间掩盖。

#### UI 重构两秒快检

在完整 20–30 分钟盲测前，先用 5 名未接触项目的目标玩家完成一次 6–8 分钟移动横屏快检。
参与者不可预读界面说明；观察者只说“请告诉我你认为发生了什么，以及你下一步会点哪里”，
不得指出按钮、图标含义或正确答案。每个状态稳定显示后立即开始计时：

1. 标题：指出题材局势和唯一进入动作；
2. 工厂：指出当前战争目标，并直接点击一个自然的准备或出击入口；
3. 战区：指出当前选中目标、风险和下一行动；
4. 战斗：在预警出现时指出威胁来自哪里、准备使用哪个动作；随后自行找到暂停；
5. 失败或 Boss 简报：指出失败原因，以及准备、试探或恢复路径；
6. 设置：自行调整一个体验选项，并说明存档/删除为什么与体验设置分开。

每个状态记录首个注视区域、首次口头答案时间、首次点击时间、误触次数、是否需要帮助，以及底栏、
手指或浏览器安全区是否遮挡。两秒门槛只评价“目标 / 风险 / 下一动作”的首次口头识别，不把完成
动画、读取详细数值或实际点击耗时塞进同一指标。通过条件：

- 5 人全部使用真实手机横屏；至少覆盖一台 Android Chrome 和一台 iPhone Safari；
- 4/5 在工厂、战区和战斗三种核心状态中，均能在 2 秒内正确说出当前目标与下一动作；
- 4/5 在有明确风险的战区、战斗或失败状态中，能在 2 秒内指出风险；
- 没有关键动作被安全区、底栏或手指稳定遮住，也没有破坏性动作因纯图标而被误触；
- 任一核心状态若有 2 人误认同一图标、点击同一错误层级或超过 5 秒，建立具体 UI finding 后复测。

使用 `docs/templates/mobile-ui-player-learning-observation.md` 每人复制一份。参与者若自愿开启
“设置 → 存档 → 本地试玩报告”，结束时一并导出 JSON；报告只辅助定位行为，不替代口头理解记录。
...
- 独立新档首章 smoke：从 1-1 连续经历 1-4 首败、信号图纸十连、研究所定向研发、反攻、工业投产、
  二星成长、1-5 Boss 胜利与第二章侦察；技能卡触控、持久化状态和各转折结算页均留证；
