...
`cleared_stages` 作为破坎成功条件；胜利页需保留援军贡献与“选择工业支援”出口。工业成长与
Boss 的连续 Web UI 旅程仍需沿同一原则扩展。工业段必须从三种玩家可见设施中作出真实选择，
完成 3D 网格选址、施工和验收，领取预置的一分钟首批产出，再通过行动投影进入冲锋/装甲二星
二选一；通过条件包含设施等级与位置、订单清空、所选资源正增量和具体角色 `star == 2`。零库存
设施不得继续提供可点击的收取行动。Boss 战斗与首章完成仍是下一段连续 Web 门禁。
