...
首墙 smoke 必须使用独立新 Chrome profile，从标题页经基地真实触控进入 1-1，再通过每次真实
...
无运行时异常、非预期 console error 或网络失败。脚本保存首战、两次压力升级和高墙首败转折；
重复技能卡触控只证明真实输入路径被执行，主动技能是否被玩家理解和成功释放仍由真人协议与
本地试玩报告交叉确认。后半章的研究所、十连、编队、反攻、工业成长与 Boss 仍需沿同一原则扩展。
