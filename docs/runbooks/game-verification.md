...
同一新档随后必须通过 1-5 Boss、只产生一次 1-5 尝试并解锁但不自动挑战 2-1；首章结算必须先
进入行动页领取一次性开服庆典礼包、开启后勤箱，并把推荐的未升星首章援军用黑金核心补至 2★；
完成后唯一主目标才切换为“领取阵营起手十连”，且必须直接落到可领取状态，不得再受隐藏指挥官
等级门槛阻断。脚本需真实领取免费十连、持久化独立 claim key、让玩家从两名同评级候选中明确选择阵营核心，并打开所选核心的
科技图纸；截至此处 2-1 尝试仍须为 0。随后必须通过真实倒计时完成所选图纸研发，把永久角色编入
空槽，并以同一存档连续通过 2-1、2-2、2-3；1★阵营核心必须在 2-4，或高掷通过时在 2-5，形成
一次真实压力测试。战报的唯一主操作必须先把玩家带到该核心的成员培养页，再以该型号专属碎片
完成 2★，继续升级到 Lv.3、通过 2-5、激活 Tier 1 并停在 3-1 侦察；由 IndexedDB 证明礼包、
箱子、福利核心、角色身份、碎片余额和成长状态均落盘。候选 `40cdbcf` 的 Chrome 150 隔离新档
旅程用时 944.737 秒：选择 `sonic`，60 枚音波碎片对 20 枚成本；全程
0 runtime exception、0 非预期 console error、0 failed request。旅程共执行 519 次角色技能卡
触控与 125 次全队爆发触控，合计 644 次、平均 0.682 次/秒；Boss
只通过玩家可见的全队爆发按钮处理，不再依赖高频逐卡输入。该频率上限证明浏览器连续可达、
目标导航、操作可触达和持久化闭环，不证明真人理解、沉浸、乐趣或继续游玩意愿。
