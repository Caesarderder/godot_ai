...
按 vertical slice 落地 Godot 2D 游戏，并让写入者、审查者和验证者职责分离。
...
2. 只修改批准 owner 的 `project-a/game/**`、`project-a/tests/**`、`project-a/tools/**` 及允许的工程配置；默认不得修改 `project-a/addons/godot_ai/**` 或暂停的 `taptap/**`。
3. 静态配置、运行状态、UI 投影和 command executor 保持分层。
4. 先写或冻结行为测试，再实现最小 vertical slice。
5. 实施者先运行 GDScript diagnostics、focused GUT 与 headless 检查，再运行 milestone verifier；不得自批。
