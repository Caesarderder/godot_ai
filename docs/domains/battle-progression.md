...
source_of_truth:
  ...
  - project-a/game/scripts/domain/battle
  - project-a/tests/unit/battle
validated_by:
  - code-review
  - manual-project-scan
tags:
...
> 当前实现与验收均以 `project-a/` Godot 为准。M3 相关战斗代码和测试路径已经出现，但稳定 seed、独立 BattleSession、跨帧率 digest 与 paired 1000 必须由完整门禁确认，因此状态仍为 `draft`。
...
- 首版验收聚焦 1-1 到 1-5，其中 1-3 和 1-5 负责失败后成长再胜验证；暂停的 TapTap 30 关原型不属于当前证据。
...
使用 Godot headless/GUT 验证 hash/tie-break golden、30/60/120 FPS digest、同一 manifest 的 paired 1000 与绝对胜率区间。
