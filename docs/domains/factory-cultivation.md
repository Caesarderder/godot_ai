...
让可点击的 3D 工厂把不同产线的产出汇入同一工业材料库存，并由该库存承担设施建造与升级；研究所提供角色
研究入口，但角色等级、升星和技能研究不再消耗工业材料。

## 职责

- 指挥中心控制设施、角色与章节功能上限；
- 新设施通过“选择建筑类型 → 打开网格 → 选择空格 → 二次确认”开工；确认前的预览不扣款、不写存档，
  确认后进入有结束时间的施工队列；首章可建设施首次施工统一为 5 秒，倒计时按秒显示并明确提示
  “完成后点击验收”，到点验收后才落成并启用；
- 已建设施的网格坐标属于工厂持久状态，越界或占用格不得提交；
- 首次工业备战的任务入口直接进入三座资源设施选择，显示当前库存与紧邻成长需求；建成后任务
  入口定位到该设施的投产收取，不得循环刷新行动页或让非资源设施抢占首次选择；
- 回收厂、精炼车间与能源站都生产工业材料，保留独立建筑、派驻和生产锚点，但不再建立三套库存；
- 新设施建造和升级只消耗工业材料，不消耗金币或工业技术；
- 三座资源建筑拥有独立生产锚点与可领取状态，点击建筑只领取其自身产出的工业材料；
- 指挥与研究建筑点击后进入各自管理面板，建筑等级和当前选择在 3D 场景中可见；
- 新档直接取得研究所建造资格，初始 30 工业材料只够完成这一座设施；玩家完成选址、施工与验收后，
  解锁新手启程礼包。1-3 首通再解锁含 30 工业材料的新游补给礼包，承担下一座资源设施预算；
- 点击研究所进入带主干、四条树枝和前后节点连线的科技蓝图 UI，展示目录中的全部马桶人型号；
  调试阶段所有蓝图统一进入 5 秒研发队列，到点领取后才确定性授予对应永久角色，
  随后通过现有六槽编队命令上阵；
- 研究所界面在研究核心附近展示四种核心资源余额；基础蓝图与突破
  十连必须在资源条和操作节点旁同时说明本次需要为 0、余额不变，且调试阶段只有 5 秒时间成本，不得让
  玩家把免费研究误解为隐藏扣费；
- 后续型号节点继续由各自进度来源开放；研究所同时承担技能与星级研究；
...
- UI 不能直接修改资源和设施状态；
- 研究所不得自动落成；玩家必须主动建成，分阶段礼包必须手动领取且 exact-once；
