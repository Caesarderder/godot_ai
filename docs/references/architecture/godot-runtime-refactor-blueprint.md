...
减少动态模式仍保留文字确认。战斗中的技能接受/未就绪反馈也只能写入 `BattleHudScreen`
状态行：App Shell 不得调用覆盖在整个 `ui_root` 顶部的全局 toast，因为它会遮住阶段目标和
五秒炮击预警；预警出现时仍抢占技能确认与拒绝反馈，预警消失后再恢复普通战术信息。实现顺序是先证明本地信号携带 detached
事件，再验证 HUD 真实汇总，最后跑首章战斗和 844×390 截图。

压炮成功不能在造成伤害的同一 5Hz tick 内从 snapshot 消失。`BattleSession` 仍在达到阈值的
当 tick 原子记录 `cannon_suppressed`、增加统计并取消该次炮击，但把该 warning 标记为
`suppressed`，固定保留 3 tick（0.6 秒）后再移除；保留期间不重复累计、不产生 impact。
`BattleHudScreen` 只读取该 accepted 状态并显示“巨炮已压制 · 安全窗口”，不能自行延时或猜测
伤害阈值。这个最短确认窗只提高反馈可读性，不改变五秒反应窗、伤害、奖励或胜负结算。
...
工厂施工倒计时属于当前 `FactoryScreen` 的表现刷新，不是领域时钟：App Shell 持有一个本地
一秒 Timer，只在基地页面且施工存在时用当前权威 state 重新 configure 已实例化的工厂 UI。
...
`HelpScreen` 把 1-4 设计卡点、信号招募免费图纸十连、研究所定向研发、冲锋/装甲两条路线、战后完全恢复、
...
路由到军团培养或目标关卡。
`BlueprintScreen` 只接管四条研究分支和已拥有图纸的研究节点；基础免费十连与正式招募统一位于
`LegionScreen` 信号招募页，抽取结果只入库设计图纸或重复设计数据。App Shell 只投影权威状态并
路由研究开始、领取、军团查看和返回命令。
