...
默认手动技能下六条路线在 2-4 遇到后段墙，一条声波路线以全程及时释放技能赢得章节捷径；
...
2-4 现只缩短暴露后的最终据点核心 60%，不改变守军、声塔或共振压力；默认手动技能的 7-seed
成熟 2-4 收敛为 307–630 tick（61.4–126.0 秒），成熟 2-5 为 445–898 tick（89.0–179.6 秒）。
六条 1★路线仍在 2-4 遇到真实成长墙，一条声波路线可凭每次技能及时释放直接完成章节，避免把
成长墙做成隐藏强制失败；自动测试不据此要求真人持续高频点击。
同一测试按首次成长墙、研发 5 秒、交互预算与实际战斗 tick 计算玩家关键路径；无墙路线只累计
实际打过的五关，不重复虚构成熟战斗，当前区间为 461–656 秒，低于 20 分钟结构上限。成熟 2-4
每场实际释放 26–38 次技能，2-5 为 34–58 次，属于待真人验证的操作负担，不能以自动请求宣称心流。
...
证明手动技能状态、就绪卡、预警和 40 点实际损失在横屏小屏可见。
