...
成长后的 2★Lv2 均通过 2-4、2★Lv3 均通过 2-5。最新招募结果由
...
第三章原本只有遭遇变化，没有新的科技成长兑现。现在 3-5 首通会把同一阵营协议派生升级为
Tier 2：快攻/防线核心分别把能量或较小护盾扩散到全部永久主力，轰炸/干扰把标定或虚弱推进至
前两个战区。`run_battle_tests.gd` 分别验证四条扩展边界及 tier 结算事实，
`run_post_30m_ui_journey_tests.gd` 验证科技蓝图从 durable `stage_3_5` 首通恢复 Tier 2；
`ui-faction-tier-two-tech-844x390.png` 与 `ui-faction-tier-two-battle-844x390.png` 证明
蓝图和 4-1 开局在基准横屏连续可见。该升级仍不新增存档字段或静态 CP。
