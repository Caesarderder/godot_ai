...
- 当前存档结构为 schema v10，继续迁移 schema v5/v6/v7/v8/v9；v8→v9 一次性归并旧资源，
  随后 v9→v10 一次性把旧四维换算为未乘星级的生命、攻击、防御、速度、暴击；新存档内容合同为
...
- 首章后推荐战力已按玩家“卡点过密”反馈放宽为 4–5 关推进窗：2-1 至 2-5 为
  ...
  2-5/3-5/4-5/5-5 四个章节 Boss。第二章起 Boss 的炮压与低战力核心过载由关卡显式配置，
...
- 首章免费十张基础设计已迁到信号招募：`ResearchBreakthroughCardDefinition` 与十个 `.tres`
  ...
  推进长期保底。研究所落成不再自动发现图纸；玩家逐张完成 45 秒研发后，`FactoryService`
...
- 当前首次设施建造合同已根据玩家“等待过长且不知道下一步”的反馈统一缩短为 5 秒。设施目录和
  ...
  30–75 秒施工也以“开工后 5 秒”作为有效完成时间，升级订单不受影响。旧 75/30 秒 Chrome 旅程
...
当前纵向切片已实现“Gman 连胜—1-4 首败—信号图纸十连—研究所逐张研发—三人反攻—资源设施投产与首批收取—
...
Lv10 前隐藏周任务。军团页呈现 B/A/S 概率、十抽 A、60 抽 S、定向保证、招募券和重复图纸转军团数据；
新增角色图鉴展示未获图纸、待研发、已入列三态。信号结果不能直接部署，必须到研究所完成图纸研发；
已研发英雄才可进入六槽编队。首章另有一次免费基础图纸十连，由 durable receipt 与存档 claim key
防重，不消耗券、不推进长期保底。
正式招募仍同时检查首章 1-5 完成与 Lv4，避免确定性教学闭环前出现随机全池入口。schema v9 保存周期 generation、领取状态、等级领取账本和招募双保底，并可
从 v5/v6/v7/v8 原地迁移。旧 `QuestService`、`AchievementService`、`WarMeritTrack` 和
