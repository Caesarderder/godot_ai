...
继续通过，证明新增阅读题没有破坏 1★后段成长墙与 2★通关价值。
...
2-4 现只缩短暴露后的最终据点核心 25%，不改变守军、声塔或共振压力；7-seed 的成熟 2-4
收敛为 395–823 tick（79.0–164.6 秒），1★仍全部在 2-4/2-5 遇到至少一次真实成长墙。
`run_post_30m_faction_tests.gd` 同时按首次成长墙、研发 5 秒、交互预算与实际战斗 tick 计算玩家
关键路径，当前为 541–696 秒，低于 20 分钟结构上限；该数值证明没有强制拖延，不代表真人会在
同样时间内阅读、比较和形成阵营认同。
