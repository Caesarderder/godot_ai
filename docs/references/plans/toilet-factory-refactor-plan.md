...
当前 revision `127323a` 的可复现候选压缩首载为 27.62 MiB，满足 30 MiB 硬门禁但距 20 MiB
目标仍差 7.62 MiB；其中 PCK 为 18.92 MiB。仓库已建立与 Godot 覆盖测试一致的 981 字符
确定性清单（886 个非 ASCII，digest `230e6abbd3ce12426a8ed2906839b24c341c6334197733a798ced8fbbf2a1764`）；
16.06 MiB 完整 Noto CJK 源字体是最大单项，正式裁剪仍需获准在隔离环境安装官方 `fonttools`。
...
使用默认关闭的本地试玩报告记录白名单事件和相对时间；报告派生 12 个首章里程碑、相邻里程碑
耗时、下一缺失节点、关卡尝试、失败命令、纯导航连跳和排除战斗后的最长停滞。研究所建造、
免费突破、三人反攻编队、资源设施投产、首次收取与升星分别可定位，但不记录命令 payload、账号、
设备、网络地址、自由文本或完整存档。
