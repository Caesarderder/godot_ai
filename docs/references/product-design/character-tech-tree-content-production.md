...
|---|---|---|---|---|
| 信号净化 | 净化 | 第三章控制只有承伤/续航回退，缺少读招反制 | 3-2 | 统一可驱散状态来源 |
| 锚桩堡垒 | 锚定 | 换位缺少保持阵型的主动选择 | 3-3 | 显式位移事件 |
| 磁轨牵引 | 聚拢 | 分线守军与AOE之间缺少位置协同 | 4-1 | 稳定敌军身份与位移免疫 |
| 相位钻袭 | 绕后 | 后排结构主要只由远程火力处理 | 4-2 | 临时离阵、返回和反潜窗口 |
| 协议编织 | 夺取 | 精英强化只能硬拆，缺少风险型反转 | 4-4 | buff白名单、来源和敌我映射 |
