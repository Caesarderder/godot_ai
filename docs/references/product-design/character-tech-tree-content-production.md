...
第二章 Boss 首通后，研发说明会让位给抽取核心对应的 Tier 1 阵营协议。协议由 2-5 首通事实与
durable 十连核心共同派生，从 3-1 起真实改变战斗开局：快攻提前爆发、防线获得开局盾、轰炸标定
首区结构、干扰虚弱首区守军。它不增加静态 CP 或隐藏常驻乘区；HUD 与结算必须显示真实触发对象，
避免科技树再次退化成无法兑现的预览文案。
第三章 Boss 首通后，同一节点开放 Tier 2 二选一：“全队协同”扩大受益对象，“阵营专精”牺牲
覆盖换取更高强度或持续时间。后续角色设计必须同时检查其阵营归属在两条 Tier 2 路线下的受益对象
和边界，避免出现无条件占优路线，也避免新角色因同时属于多个隐性阵营而重复触发协议。
战区侦察会进一步用当前编队对照每关已发布的主解与备选角色，显示“已覆盖 / 待补”；角色推荐
因此成为可执行换阵判断，而不是与玩家实际角色池无关的静态文案。该投影只解释现有规则，不提供
隐藏克制倍率。
...
| 角色 | 评级 | 职责 | 1★ | 2★ | 3★ |
|---|---|---|---:|---:|---:|
| 破城手 | B | 突破 | 1724 | 2139 | 2574 |
| 震音师 | A | 控场 | 1644 | 2042 | 2460 |
| 火箭炮手 | B | 攻城 | 1419 | 1736 | 2053 |
| 爆破手 | A | 爆发 | 1419 | 1736 | 2053 |
| 铁甲撞手 | A | 承压 | 1638 | 2049 | 2460 |
| 双锯手 | S | 斩杀 | 2294 | 2893 | 3502 |
| 修补匠 | B | 续航 | 1638 | 2049 | 2460 |
| 寄生母巢 | S | 召唤 | 2168 | 2748 | 3338 |
...
当前 60 关具有单调递增推荐战力、三关循环、战力墙、科技墙、章节 Boss、角色推荐和非付费回退路线。
...
60 关推荐曲线，验证新角色规格，并输出实现与关卡投放合同。

## 示例试产

示例“A级磁牵手”使用“聚拢”作为新战术动词：
...
角色知识库与运行时已新增 5 个角色，均复用当前四阵营与四条科技分支，不新增货币或隐藏克制：

| 角色 | 动词 | 主要内容缺口 | 首次安全练习 | 实施依赖 |
|---|---|---|---|---|
| 静频师 | 净化 | 第三章控制只有承伤/续航回退，缺少读招反制 | 3-2 | 统一可驱散状态来源 |
| 锚堡 | 锚定 | 换位缺少保持阵型的主动选择 | 3-3 | 显式位移事件 |
| 磁牵手 | 聚拢 | 分线守军与AOE之间缺少位置协同 | 4-1 | 稳定敌军身份与位移免疫 |
| 钻地客 | 绕后 | 后排结构主要只由远程火力处理 | 4-2 | 临时离阵、返回和反潜窗口 |
| 夺频母巢 | 夺取 | 精英强化只能硬拆，缺少风险型反转 | 4-4 | buff白名单、来源和敌我映射 |

详细数值、培养、四段教学、回退、表现、测试和回滚阈值见
[角色表与知识地图索引](../characters/index.md)。当前已有目录、长期招募、确定性关卡图纸、研究、
编队、技能、科技树和程序化表现证据；真人理解、偏好与长期平衡仍为 unknown。
