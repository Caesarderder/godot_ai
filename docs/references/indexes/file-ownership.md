...
> `project-a/` 是当前 Godot Web-first 3D 实现根。v4 Meta、工厂、培育、六人编队、任务/成就目标中心和三阶段战斗内核已落到 `game/scripts/**`，其余不存在路径仍是 planned ownership。
...
| `project-a/scripts/main.gd` | application-shell | 标题、营地、目标中心、工厂、培育、六人编队、出征、战斗 HUD、结算生命周期 | present / 1280×720 design canvas + 844×390 capture |
...
| `project-a/game/scripts/ui/**` | presentation | 可复用独立 UI 场景/组件 | absent；当前营地、目标中心和战斗 UI 在 App Shell |
